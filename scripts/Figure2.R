...
scopus.avg<-predict(m.citations, newdata=pred2, re.form = NA)


## load altmetric data
load('data/altmetric_lmer_fit.Rdata')

## pred lines for average mention rate
pred2<-expand.grid(OA =unique(predicted.mentions$OA), SJRfac.scaled=0, Year = 2007, Source.title='Nature')
...
axis(1, at=sort(unique(pred$jour.bin.scaled)), labels=c('Low', 'Medium', 'High', 'Very high'), cex.axis=cx.ax)
...
clip(-10, max(predicted.mentions$xlim), 0, 100)
...
with(predicted.mentions[predicted.mentions$source=='news' & predicted.mentions$OA=='Closed',], 
		plotCI(xlim, p, ui=p, li=p, pch=19,cex=1.5, sfrac=0, 
			axes=F, xlim=c(-1.5, 1.5), xlab='', ylab='', ylim=c(0, 1.8),
			scol=cols[1], col=cols[1]))
with(predicted.mentions[predicted.mentions$source=='news' & predicted.mentions$OA=='Open',], 
		plotCI(xlim, p, ui=p, li=p, pch=19,cex=1.5, sfrac=0,
			scol=cols[2], col=cols[2], add=TRUE))
axis(1, at=unique(predicted.mentions$SJRfac.scaled), labels=c('Low', 'Medium', 'High', 'Very high'), cex.axis=cx.ax)
...
## twitter
par(mar=c(1.5,1.5, 1.5, 1.5), xpd=FALSE)
with(predicted.mentions[predicted.mentions$source=='twitter' & predicted.mentions$OA=='Closed',], 
		plotCI(xlim, p, ui=p, li=p, pch=19,cex=1.5, sfrac=0, 
			axes=F, xlim=c(-1.5, 1.5), xlab='', ylab='', ylim=c(0,10),
			scol=cols[1], col=cols[1]))
with(predicted.mentions[predicted.mentions$source=='twitter' & predicted.mentions$OA=='Open',], 
		plotCI(xlim, p, ui=p, li=p, pch=19,cex=1.5, sfrac=0,
			scol=cols[2], col=cols[2], add=TRUE))
axis(1, at=unique(predicted.mentions$SJRfac.scaled), labels=c('Low', 'Medium', 'High', 'Very high'), cex.axis=cx.ax)
axis(2, cex.axis=cx.ax)
...
## policy
par(mar=c(1.5,1.5, 1.5, 1.5), xpd=FALSE)
with(predicted.mentions[predicted.mentions$source=='policy' & predicted.mentions$OA=='Closed',], 
		plotCI(xlim, p, ui=p, li=p, pch=19,cex=1.5, sfrac=0, 
			axes=F, xlim=c(-1.5, 1.5), xlab='', ylab='', ylim=c(0.1,0.7),
			scol=cols[1], col=cols[1]))
with(predicted.mentions[predicted.mentions$source=='policy' & predicted.mentions$OA=='Open',], 
		plotCI(xlim, p, ui=p, li=p, pch=19,cex=1.5, sfrac=0,
			scol=cols[2], col=cols[2], add=TRUE))
axis(1, at=unique(predicted.mentions$SJRfac.scaled), labels=c('Low', 'Medium', 'High', 'Very high'), cex.axis=cx.ax)
axis(2, cex.axis=cx.ax)
